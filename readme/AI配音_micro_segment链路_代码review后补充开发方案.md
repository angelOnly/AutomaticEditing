...
```text
1. asr_micro_segment 仍然直接读 source_aggregate.json 内嵌 raw_asr_index，未优先读 source_raw_asr_index.json 文件。
2. legacy timeline_digest fallback 函数仍保留，配置误开后会退回旧链路。
3. asr_micro_segment input_hash 没有显式绑定 raw_asr_index 文件和 prompt version。
...
7. ai_voiceover_candidate_materialize 对 overlong candidate 默认不失败。
8. tts / cut_plan 需要继续确认并补强空结果 fail-fast。
9. run_web.py 端口探测建议修正 0.0.0.0 探测问题。
```
...
# P0-1. asr_micro_segment 必须优先读取 source_raw_asr_index.json 独立文件
...
def _load_source_raw_asr_index(self) -> dict[str, Any]:
    ...
    找不到时 fail-fast，不回退 timeline_digest.speech。
    """
...
# P1-4. ai_voiceover_candidate_materialize 默认对 overlong fail-fast
...
```text
commit 1: asr_micro_segment raw_asr_index 文件加载与 hash 修正
commit 2: 禁止 timeline_digest fallback 与 deprecated candidate materialize 防误用
commit 3: source_aggregate ASR/chunk diagnostics 补强
commit 4: content_analysis 输入瘦身与协议检查
commit 5: candidate overlong fail-fast 默认策略
commit 6: voiceover_script / tts / cut_plan 空结果 fail-fast 补强
commit 7: run_web 端口探测和 Web step label 优化
```
