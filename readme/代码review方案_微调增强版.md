...
> 建议实际函数名可以改成 `_short_video_edit_plan_has_valid_editing_structure()`，因为真正要判断的不是 scripts 字段是否存在，而是 editing_structure 里有没有可回查到 candidate_clip 的有效 `source_clip_id`。下面保留原函数名，方便在原方案上微调。
