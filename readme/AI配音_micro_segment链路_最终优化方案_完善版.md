...
```text
preferred_max_clip_seconds 是新增代码协议，不只是新增配置项。
如果 pipeline.py 的 step_ai_voiceover_candidate_materialize() 不读取它，
那么配置里写 preferred_max_clip_seconds 不会产生任何效果。

同时，代码中的默认值也要和本配置策略保持一致。
例如 hard_max_segment_seconds 的代码默认值不应继续回退到 45 秒，
否则配置缺失、测试环境配置不完整时，会和“35 秒封顶”的原则不一致。
```
...
```text
preferred_max_clip_seconds 必须进入 input_hash / debug.config / diagnostics 逻辑。
否则 rerun/reuse 时可能因为缓存没有感知配置变化，导致看似改了配置但实际仍复用旧结果。
```
...
注意：如果 `truncate_overlong = true` 只允许作为 debug / 临时排查选项，不建议作为默认生产策略。即使启用截断，也应该先记录原始 `raw_duration`，再记录截断后的 `duration_seconds`，避免 debug 时看不出原始片段为什么超长。
...
这一项不是可选优化，而是必须修复项。因为 candidate_clip 的超长问题虽然发生在物化阶段，但根源之一可能是 micro_segment 在“拆长段后又合并短段”时重新突破 hard_max。
...
同时，函数末尾“最后一个短段向前合并”的逻辑也要加同样的 `hard_max` 判断，不能只修前向合并，不修尾段回并。否则最后一个短片段仍可能把前一个合法片段重新合并成超长片段。
...
另外，`_enforce_micro_segment_limits()` 里的默认值要和配置文档一致：

```text
hard_max_segment_seconds 缺省值建议使用 35，而不是 45。
min_segment_seconds 缺省值建议使用 4 或与 config.toml 保持一致。
```

这样可以避免某些测试环境、迁移环境、旧配置文件漏配时，代码默认策略和产品策略不一致。
...
```text
1. content_analysis_micro_segment.txt 的规则里；
2. _build_content_analysis_micro_segment_text() 构造的输入说明里。
```

如果两处都补，要避免重复啰嗦，核心是让模型知道“偏长片段慎选”，而不是让它拥有改时间的权限。
...
这条规则只影响模型如何选择 clip，不允许模型输出新的时间码、source_start/source_end 或自造 clip_id。长片段是否允许进入候选池，仍由 `ai_voiceover_candidate_materialize` 的软硬阈值控制。
...
这次排查中还暴露一个问题：失败任务必须能看到失败步骤的完整 debug 产物。

当前 `ai_voiceover_candidate_materialize` 应该先写出 `candidate_clips.json` 和 `materialize_debug.json`，再根据 fail_reasons 抛错。这个方向是对的，关键是 Web 侧必须能看到这些文件。

建议检查 Web 文件树 / 文件读取 / 如存在则检查打包下载逻辑，确保失败时也能访问：
...
如果失败步骤已经写出 debug，但 Web 文件树或压缩包下载看不到，才需要修 Web 侧文件展示或打包清单。

---

## 9.1 Web 启动方式与统一 source 链路边界

本方案默认优先保障当前 Web 启动方式：
...
```text
source_prepare -> source_analysis -> source_aggregate -> asr_micro_segment -> content_analysis -> ai_voiceover_candidate_materialize
```

需要注意：

```text
1. run_web.py 只是启动 Web 服务，不直接决定 AI 配音链路细节。
2. AI 配音 micro_segment 主链路依赖 source_aggregate 输出的 source_raw_asr_index。
3. 旧的非统一单视频 run_pipeline.py --input 路径，需要单独确认 workflow 是否包含 ai_voiceover_candidate_materialize。
4. 如果旧链路没有 source_aggregate / source_raw_asr_index，不应该回退到 timeline_digest 反推时间。
```

所以这次修复不要为了兼容旧单视频链路重新打开：

```toml
allow_asr_micro_segment_from_timeline_digest = true
allow_ai_voiceover_candidate_from_content_analysis = true
allow_legacy_ai_voiceover_chunk_content_analysis = true
```

旧链路如果还要支持，应该另做兼容设计，而不是污染当前 AI 配音主链路。
...
input_hash / debug.config 纳入 preferred_max_clip_seconds
```

### 第三步：修 micro_segment 合并逻辑（必须项）
...
尾段向前合并也必须检查 hard_max_segment_seconds。
合并后做二次校验。
代码默认 hard_max 与 config.toml 保持一致。
```
...
```text
python run_web.py 启动方式不变。
Web 侧 AI 配音任务仍走统一 source_prepare -> source_analysis -> source_aggregate。
失败时 Web 文件树能看到 ai_voiceover_candidates/v*/materialize_debug.json。
旧的 timeline_digest fallback 不应被重新打开。
```
...
失败时必须能看到可排查 debug。
preferred_max_clip_seconds 必须由代码实际消费，不能只写配置。
micro_segment 拆分后再合并，仍不能突破 hard_max_segment_seconds。
```
