...
2. 模型返回空 groups 时直接 fail-fast。
3. 如果将来临时排查需要 fallback，必须通过 config 显式开启。
4. fallback_used 必须写入 diagnostics，不能伪装成正常结果。
```
...
```text
1. 不在 candidate materialize 阶段截断 source_end。
2. 超长只写 diagnostics。
...
## 7.2 新增函数 `_short_video_edit_plan_has_scripts()`

放在 `_materialize_short_video_edit_plan()` 附近：

```python
def _short_video_edit_plan_has_scripts(self, plan: Any) -> bool:
...
if (
    ...
    and semantic_retry
    and not self._short_video_edit_plan_has_scripts(materialized)
):
    for retry_index in range(retry_rounds):
        ...
        if self._short_video_edit_plan_has_scripts(retry_materialized):
...
在 `step_asr_micro_segment()` 开头加：

```python
if self.options.production_mode == "ai_voiceover" and not self._is_virtual_source_manifest():
    compat = self.config.raw.get("compat", {})
...
def _validate_ai_voiceover_micro_segment_chain(self) -> dict[str, Any]:
    ...
    raw_asr = self._load_optional_step_json("source_aggregate", {}).get("raw_asr_index", {})
    raw_count = int(raw_asr.get("raw_asr_segment_count") or 0) if isinstance(raw_asr, dict) else 0
    if raw_count <= 0:
        hard_issues.append("source_raw_asr_index.raw_asr_segment_count is 0")
...
建议在 `step_voiceover_script()` 成功校验后调用：
...
```text
1. 修 asr_micro_segment fallback。
2. 修 candidate_materialize 不截断。
3. 统一 content_analysis micro_segment_id 文案。
4. 加 raw_asr diagnostics。
5. 加 max_sentence_chars 切分。
6. 隔离 legacy content_analysis candidate 函数。
7. 加 short_video_edit_plan 语义 retry。
8. 加 legacy CLI 单源阻断提示。
9. 加链路自检。
```
...
2. candidate_materialize 不截断。
3. content_analysis 统一 micro_segment_id。
```

这三项不改，后面即使能跑通，也还是会出现“表面 success，但语义不可信”的问题。
