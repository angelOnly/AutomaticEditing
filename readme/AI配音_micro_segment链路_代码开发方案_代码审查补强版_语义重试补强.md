...
> 本次输出为“代码审查补强版”：保留原方案结构，只补充字段协议、fallback 删除、候选物化边界、缓存 hash 与 fail-fast 细节。
...
本轮补强不是推翻原方案，而是在原代码开发方案上补充几个容易被开发误实现的细节。

必须特别注意：当前仓库里已经存在一套“半成品 micro_segment 兼容逻辑”，包括：

```text
1. content_analysis 会在 asr_micro_segment 不成功时 fallback 到 chunk brief 输入。
2. content_analysis 内部会直接把 segment/chunk 物化成 candidate_clips。
3. short_video_edit_plan 在模型空输出时会自动选前 N 个 clip。
4. voiceover_script 在模型空输出或异常时会 fallback 生成文案。
5. tts 在 outputs=[] 且 failed=0 时可能仍记录 success。
```

本次开发不是简单“新增 raw_asr_index 和 ai_voiceover_candidate_materialize”即可，而是要把这些旧兜底从 AI 配音统一多源主链路里明确删除或禁用。否则会形成新旧链路并存：表面跑通，实际仍可能空结果一路 success，最后才在 `news_quality_gate` 暴露为泛化错误。

本轮补强后的主原则：

```text
1. content_analysis 只做 selected_segments，不再产出 candidate_clips。
2. ai_voiceover_candidate_materialize 是 AI 配音 candidate_clips 的唯一生成点。
3. short_video_edit_plan 只读 ai_voiceover_candidate_materialize。
4. voiceover_script 不允许静默 fallback。
5. require_tts=true 时，tts.outputs 为空或 success=0 必须失败。
6. 所有字段协议以 micro_segment_id 为主键；segment_id 只作为兼容输入，不作为新版主协议。
7. 区分“LLM 调用级重试”和“业务语义重试”：合法 JSON 但 videos/clip_ids/scripts 为空，不会触发 llm.py 的 max_retries，必须在业务层显式重试或 fail-fast。
```

补充说明：

```text
当前 llm.py 的 max_retries 只覆盖 API 异常、超时、非合法 JSON、JSON repair 失败等调用级问题。
如果模型返回 []、{"videos": []}、{"scripts": []} 这类合法 JSON，LLM 层会视为成功返回。
因此 short_video_edit_plan / voiceover_script 这类步骤必须自己检查“业务结果是否为空”，不能指望底层 max_retries 自动重试。
```
...
2. source_aggregate 的 input_hash 必须纳入 child asr output 的路径、hash 或 child manifest 中 asr step 的 output_hash。
   仅依赖 source_analysis hash 可能导致子任务 ASR 产物变化但 source_aggregate 被错误复用。
   这不是优化项，而是缓存正确性的阻断项：如果 child ASR 变了，source_raw_asr_index.json 必须重新生成。
...
```text
1. 不是在现有 step_content_analysis() 上再加一个分支，而是要移除 AI 配音主链路里的 chunk fallback。
2. `_materialize_content_analysis_candidates_from_segments()` 不能再作为 AI 配音主链路生成 candidate_clips 的函数。
3. `_materialize_candidate_clips_from_chunks()` 不能在 AI 配音统一多源主链路中被调用。
4. 如果保留这些函数，只能作为 legacy/compat 函数，必须由显式 compat 开关控制，默认 false。
```
...
```text
1. 新版内部主键统一叫 micro_segment_id。
2. 如果提示词文档或模型临时输出 segment_id，normalize 可以兼容读取。
3. 但最终 content_analysis.json 的 selected_segments 必须统一写成 micro_segment_id。
4. 下游 ai_voiceover_candidate_materialize 只能读取 micro_segment_id，不再把 segment_id 当新版主协议继续扩散。
```
...
同时必须明确：这是 AI 配音 candidate_clips 的唯一生成步骤。`content_analysis` 只允许输出 `selected_segments`，不能继续输出或物化 `candidate_clips`。
...
#### 9.2.0 先明确：模型空输出不会自动触发 llm.py 的 3 次重试

当前底层 LLM 客户端的 3 次重试属于“调用级重试”，只在 API 调用异常、超时、返回内容无法解析成 JSON、JSON repair 后仍失败等情况下触发。

如果模型返回的是合法 JSON，但业务内容为空，例如：
...
底层 `call_json()` 会认为调用成功，不会进入第 2 次、第 3 次。后续是否重试，必须由 `step_short_video_edit_plan()` 或 `_materialize_short_video_edit_plan()` 的业务校验来决定。

因此本轮必须新增“业务语义重试 / 失败”逻辑：

```text
1. JSON 解析成功但 raw_videos 为空，不允许自动选前 N 个 clip。
2. 可以在 short_video_edit_plan 业务层重试 1-2 次。
3. 重试时应在输入中附加上次空输出诊断，明确要求必须从候选 clip_id 中选择。
4. 重试后仍为空，必须 fail-fast。
5. 不能用“前 8 个 clip”兜底替代模型选择。
```

推荐新增 helper，而不是修改 `llm.py` 的通用 retry：

```python
def _run_short_video_edit_plan_with_semantic_retry(self, text_input: str, *, max_attempts: int = 2) -> Any:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if attempt > 1:
            retry_suffix = (
                "\n\n【上一次输出无效】\n"
                f"原因：{last_reason or 'videos/clip_ids 为空'}\n"
                "请只从上面候选 clips 的 clip_id 中选择，必须输出非空 clip_ids。"
            )

        result = self._run_text_agent(
            "short_video_edit_plan",
            text_input + retry_suffix,
        )
        last_result = result

        videos = self._extract_short_video_raw_videos(result)
        if videos:
            has_clip_ids = any(
                self._coerce_id_list(v.get("clip_ids") or v.get("source_clip_ids"))
                or (isinstance(v.get("selected_clips"), list) and v.get("selected_clips"))
                for v in videos
                if isinstance(v, dict)
            )
            if has_clip_ids:
                return result
            last_reason = "videos 存在，但 clip_ids 为空"
        else:
            last_reason = "videos 为空"

    raise UserFacingPipelineError(
        "short_video_edit_plan_empty_after_retry",
        user_message="AI 配音选片规划失败：模型重试后仍没有输出有效 clip_ids。",
        suggestions=[
            "检查 short_video_edit_plan 输入候选池是否为空。",
            "检查提示词是否只允许输出 clip_ids。",
            "检查模型原始输出是否为 [] 或 videos=[]。",
        ],
        technical_detail={"last_reason": last_reason, "last_result": last_result},
    )
```

同时新增一个只负责解析 raw_videos 的 helper，避免 `_materialize_short_video_edit_plan()` 内重复写解析逻辑：

```python
def _extract_short_video_raw_videos(self, result: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(result, dict):
        ...
        return [x for x in raw_videos if isinstance(x, dict)]
    return []
```

`step_short_video_edit_plan()` 推荐改为：

```python
def step_short_video_edit_plan(self) -> None:
    text_input = self._build_short_video_edit_plan_text()
    result = self._run_short_video_edit_plan_with_semantic_retry(text_input, max_attempts=2)
...
注意：这个 semantic retry 只服务 short_video_edit_plan 的业务空输出，不要把它做成全局 LLM retry，否则会影响其他 agent 的语义。
...
如果已经在 `step_short_video_edit_plan()` 中做了 semantic retry，那么 `_materialize_short_video_edit_plan()` 里仍要保留最终兜底检查，但只能失败，不能再自动补 clip：
...
if not raw_videos:
    raise UserFacingPipelineError(
        ...
        suggestions=[
            "检查 short_video_edit_plan 提示词输出是否为数组或 {videos:[...]}。",
            "检查输出对象是否包含非空 clip_ids。",
            "查看 _llm_debug 中 semantic retry 后的最后一次原始输出。",
        ],
...
#### 10.3.1 删除 voiceover_script 静默 fallback

当前仓库实现中，如果模型没有返回 `scripts`，或者模型调用异常，可能会走 `_fallback_voiceover_script()` 继续生成文案。
...
1. 模型返回为空：失败。
2. 模型返回没有 scripts：失败。
3. 某个 script 没有 narration_segments：失败。
4. narration_segments 没覆盖全部 shot_id：失败。
5. narration_segments 出现多余 shot_id：失败。
6. narration_segments.text 为空：失败。
7. 不允许调用 _fallback_voiceover_script() 作为主链路兜底。
```

如果为了历史任务保留 `_fallback_voiceover_script()`，只能放在显式 compat 开关后面，默认关闭，并且不能用于本轮统一多源 AI 配音验收。
...
当前用户新版提示词文档已经包含 `content_analysis_micro_segment`，但仓库代码目前还没有把它正确接入。新版代码协议必须依赖它。
...
```text
如果提示词文件当前写的是 segment_id，开发时要同步改成 micro_segment_id。
如果暂时不改提示词，代码 normalize 可以兼容读取 segment_id，但最终落盘必须统一成 micro_segment_id。
不要让 segment_id 和 micro_segment_id 在下游同时扩散。
```
...
7. step_short_video_edit_plan() 是否区分调用级 retry 与业务语义 retry：合法 JSON 但 videos/clip_ids 为空时是否会重试或 fail-fast。
8. short_video_edit_plan 的 semantic retry 是否不会修改 llm.py 全局 max_retries 语义，避免影响其他 agent。
9. step_voiceover_script() 是否校验 shot_id 覆盖和 text 非空。
10. step_tts / cut_plan 是否对空 scripts、空 narration_segments、空 outputs、空 clips fail-fast。
11. 直接 run_pipeline.py --input 是否被明确排除在本轮验收之外，避免误判。
12. source_aggregate input_hash 是否纳入 child ASR output_hash，避免 raw_asr_index 复用旧缓存。
13. content_analysis 最终落盘字段是否统一为 micro_segment_id，而不是 segment_id / clip_id 混用。
14. voiceover_script 是否已经删除 _fallback_voiceover_script() 在主链路中的静默兜底。
```
...
4. 把合法 JSON 但业务为空的输出误认为 llm.py 已经重试过。
5. voiceover_script success 但 scripts=0。
6. tts success 但 outputs=0。
7. cut_plan success 但 clips=0。
8. voiceover_script 模型失败后自动 fallback 生成文案。
9. 新旧字段 segment_id / micro_segment_id 混用并继续扩散到下游。
```
