...
>
> 本次只在原 review 文档上继续微调补强：重点补充 workflow 实际运行模式确认、prompt 绑定校验、缓存向后失效、source duration 获取优先级、voiceover_script 语义重试、旧产物污染阻断、补充验收用例和最终验收红线。
>
> 第五次 review 补充：结合当前 `clean-highlight-reassembly` 分支的实际状态，把“尚未接入”和“已接入但需验收”的内容分开，避免 Codex 重复新增；同时补强 `candidate_materialize` 禁止为满足 `min_clip_seconds` 扩展事实边界、`content_analysis` prompt 不得继续使用 `segment_id/time`、child ASR hash 缺失时必须读取文件内容 hash、raw ASR diagnostics 必须能定位 ASR/chunk 时间错位。
...
11. 补 workflow 实际运行模式确认、prompt 绑定校验、缓存向后失效和旧产物污染阻断。
```

补充说明：如果当前分支已经部分实现了 workflow / AGENT_INFO / handler，不要重复新增；本方案后续相关小节默认按“检查现有实现是否满足验收条件”执行。

```text
已接入但必须验收：
1. AGENT_INFO["content_analysis"] 是否已经绑定 CONTENT_ANALYSIS_MICRO_SEGMENT_PROMPT。
2. ("ai_voiceover", true) workflow 是否已经包含 ai_voiceover_candidate_materialize。
3. short_video_edit_plan 依赖是否已经指向 ai_voiceover_candidate_materialize。
4. source_aggregate 是否已经输出 source_raw_asr_index.json。

已存在但必须修正：
1. asr_micro_segment 是否仍有 missing sentence fallback。
2. content_analysis 输入是否仍提示 “segment_id” 或给模型 time 时间码。
3. candidate_materialize 是否仍会截断 overlong 或为了 min_clip_seconds 扩展事实边界。
4. legacy content_analysis candidate 物化函数是否仍可能被新版 AI 配音链路误调用。

尚需补强：
1. raw_asr_index diagnostics。
2. semantic retry 的响应文件和 diagnostics。
3. stale artifact producer 校验。
4. effective_runtime_flags 和失败导出包。
```

---

# 零、先补运行模式和 prompt 绑定校验

这一节不是新增架构，只是防止开发改了主链路，但实际运行时误入旧 workflow，或者 `content_analysis` 仍然绑定旧 prompt。

## 0.1 启动时输出 resolved workflow

本轮验收主路径是 AI 配音统一多源链路，也就是：
...
```text
resolved_workflow_steps.json
```

或写入 `effective_runtime_flags.json`，至少包含：
...
必须在日志、manifest 或失败导出包中明确提示：

```text
当前进入 legacy workflow，不属于本轮 micro_segment 主链路验收范围。
```

不要在 legacy 单源链路里临时恢复 `timeline_digest.speech` fallback。

## 0.2 `content_analysis` prompt 绑定必须运行前校验
...
但在 AI 配音统一多源链路里，它的职责已经变成：

```text
micro_segment selector
```

所以运行 `step_content_analysis()` 前必须校验：

```text
1. production_mode == ai_voiceover。
2. 当前 workflow 是统一多源 AI 配音主链路。
3. 当前 prompt_key / prompt_version 明确包含 micro_segment 语义。
4. AGENT_INFO["content_analysis"] 不允许继续绑定旧 CONTENT_ANALYSIS_PROMPT。
5. 如果仍绑定旧 chunk 版 prompt，直接 fail-fast。
```
...
```text
content_analysis 当前必须绑定 AI 配音 micro_segment 版本 prompt，不能复用旧 chunk / candidate_clips 版本 CONTENT_ANALYSIS_PROMPT。
```

原因：这不是普通 prompt 文案差异，而是字段协议差异。旧 prompt 会让模型继续输出 `candidate_clips` / `chunk_id` / `segment_id` 等旧结构，后面即使能跑也会污染新版链路。
...
第五次 review 补强：这一项必须定为 P0 阻断项，不属于“诊断增强”。只要默认仍会生成 fallback group，或者 fallback group 会进入正式 `micro_segments.json`，本轮验收直接失败。
...
6. 不为了满足 min_clip_seconds 自动向后扩展事实边界；过短只记录 short_clips diagnostics，是否失败由配置控制。
```
...
## 2.2.1 禁止为了 `min_clip_seconds` 扩展事实边界

当前代码如果发现 padding 后 duration 仍小于 `min_clip_seconds`，容易写成：
...
```text
candidate_materialize 只负责把 selected micro_segment 物化为可剪辑 clip。
它不能为了凑够最小时长，把 micro_segment 后面的新事实、新句子或空镜自动吃进来。
```
...
```text
1. 默认不扩展 padded_end。
2. 默认不向前吞上一句，也不向后吞下一句。
3. 如果 fail_on_short=true 且存在 short_clips，则 candidate_materialize fail-fast。
4. 如果 fail_on_short=false，则保留原事实边界并写入 diagnostics。
5. 如果确实要补足时长，应回到 asr_micro_segment 合并/拆分策略处理，而不是 candidate 阶段扩边界。
```

`materialize_debug.json` 里必须增加：
...
## 2.6 补 source duration 获取优先级

`candidate_materialize` 加 padding 后必须做素材物理边界修正，但 source duration 不能随便猜。

建议 source duration 获取优先级：

```text
1. source_aggregate.sources[].duration_seconds
2. source_raw_asr_index.sources[].duration_seconds
3. child metadata.duration_seconds
4. 如果都没有，不允许盲目 clamp 到 0 或无限大，只记录 missing_source_duration diagnostics。
```

补充规则：

```text
1. 没有 source duration 时，仍允许 clamp start >= 0。
2. 没有 source duration 时，不能把 end clamp 到未知时长。
3. 有 source duration 时，padded_end 不得超过 source duration。
4. clamp 只修正物理边界，不承担缩短 overlong 或扩展 short clip 的职责。
```

建议 debug 增加：

```python
missing_source_duration: list[dict[str, Any]] = []
clamped_clips: list[dict[str, Any]] = []
```

`materialize_debug.json` 里也要写入：

```python
"missing_source_duration": missing_source_duration,
"clamped_clips": clamped_clips,
```

这样 render / ffmpeg 阶段如果还有越界问题，能直接回查是 duration 缺失、padding 越界，还是 micro_segment 时间本身异常。
...
2. 不再提示 segment_id。
3. 不给模型 time/source_start/source_end 时间码。
4. 输入可以保留 source_id，帮助模型区分多源素材；但输出不允许包含 source_id。
5. 只给 duration、声、画、source_id。
6. prompt 文案里不得出现 candidate_clips / chunk_id / segment_id 作为输出协议。
```
...
## 3.3 运行态禁止旧 `content_analysis` 输出污染新版链路

即使 normalize 继续兼容 `segment_id`，也必须把新版主协议写死：

```text
1. content_analysis 主输出只能是 selected_segments。
2. selected_segments 主字段只能是 micro_segment_id。
3. content_analysis 不允许继续输出 candidate_clips 给下游使用。
4. 如果模型返回 candidate_clips，只能写入 diagnostics，不能作为候选池。
5. short_video_edit_plan 只能读取 ai_voiceover_candidate_materialize 的 candidate_clips。
```

建议在 `content_analysis.json` 里补：

```json
{
  "protocol": "ai_voiceover_micro_segment_selector_v1",
  "candidate_clips_ignored": true,
  "diagnostics": {
    "legacy_segment_id_used": 0,
    "legacy_candidate_clips_ignored": 0
  }
}
```

这条主要是防止模型或旧 prompt 意外输出旧结构后，下游又被兼容逻辑带回旧链路。
...
# 七点六、给 `voiceover_script` 加轻量业务语义重试

`voiceover_script` 和 `short_video_edit_plan` 一样，也可能出现“合法 JSON 但业务为空”的情况。底层 `llm.py` 的 retry 不会处理 `{"scripts": []}`、`{"narration_segments": []}` 这种语义失败。

## 7.6.1 修改目标

```text
1. voiceover_script 返回 scripts=[] 时，不允许 fallback 默认文案。
2. narration_segments=[] 时，不允许继续进入 tts。
3. narration_segments 的 shot_id 无法对齐 editing_structure 时，视为业务无效。
4. 允许 semantic retry 一次。
5. retry 后仍无效，直接 fail-fast。
```
...
`voiceover_script` 的业务成功条件不能只看 `scripts` 非空，必须满足：

```text
1. scripts 非空。
2. 每个需要配音的视频有 narration_text 或 narration_segments。
3. narration_segments 非空。
4. narration_segments[].shot_id 能在 short_video_edit_plan.scripts[].editing_structure[].shot_id 中找到。
5. text 非空。
6. 不允许生成新的 shot_id。
```
...
```text
1. editing_structure 的 shot_id 白名单。
2. 每个 shot 对应的事实摘要 / asr_text / visual_context。
3. 上一次模型输出。
4. 上一次失败原因，例如 scripts_empty / narration_segments_empty / unknown_shot_id。
```
...
```text
timeline_digest
content_analysis 原始大输入
旧 candidate_clips fallback
自动默认文案
```

retry 成功后，最终 `voiceover_script.json` 要写：

```json
{
  "semantic_retry_used": true,
  "semantic_retry_attempt": 2
}
```

retry 失败时，必须保留 retry 的 input / raw_response / model_output，方便回查。
...
本轮改造会新增 `ai_voiceover_candidate_materialize`，同时删除旧的 content_analysis 直接产出候选的语义。开发和测试时很容易遇到任务目录里已经有旧产物，导致后续步骤误读旧 success 输出。

## 11.5.1 修改目标

```text
1. 当前 step fail-fast 后，下游不得读取历史 success 产物继续跑。
2. workflow 结构变化后，后续依赖步骤不得复用旧版本输出。
3. ai_voiceover_candidate_materialize 失败时，short_video_edit_plan 不得读取 content_analysis.candidate_clips 或旧 candidate_clips.json。
4. short_video_edit_plan 失败时，voiceover_script 不得读取旧 editing_structure。
5. voiceover_script 失败时，tts 不得读取旧 narration_segments。
```

## 11.5.2 建议实现方式

```text
1. 每个 step 输出写入 producer_step、producer_version、input_hash、protocol。
2. 下游读取时校验 producer_step 是否是允许的上游。
3. short_video_edit_plan 只接受 producer_step=ai_voiceover_candidate_materialize 的候选池。
4. voiceover_script 只接受 producer_step=short_video_edit_plan 的 editing_structure。
5. tts 只接受 producer_step=voiceover_script 的 narration_segments。
```
...
```json
{
  "protocol": "ai_voiceover_candidate_clips_v1",
  "producer_step": "ai_voiceover_candidate_materialize",
  "input_hash": "...",
  "candidate_clips": []
}
```

如果 producer 不匹配，直接 fail-fast：

```text
stale_or_invalid_upstream_artifact
```
...
## 12.1.1 semantic retry 的触发条件和次数要收紧

semantic retry 只能用于“合法 JSON 但业务结果无效”的场景，不要和底层 LLM 调用重试混在一起。

必须满足：

```text
1. semantic retry 默认最多 1 次。
2. 只在业务结果为空、clip_id 无效、shot_id 无法对齐、narration_segments 为空等情况下触发。
3. API 异常、超时、非法 JSON、JSON repair 失败，仍交给 llm.py 的调用级 retry 处理。
4. retry prompt 只能携带候选白名单、shot_id 白名单和上次错误原因。
5. retry 不能扩大输入范围，不能重新引入 timeline_digest 或旧 content_analysis fallback。
6. retry 失败必须 fail-fast，不能降级 success。
```
...
## 12.2.1 raw ASR 变化后必须向后击穿下游缓存

`source_aggregate` 的 input_hash 纳入 child ASR 只是第一层，还必须确保后续步骤的 input_hash 连带变化。

必须满足：

```text
1. source_raw_asr_index.json 变化后，asr_micro_segment input_hash 必须变化。
2. asr_micro_segment 输出变化后，content_analysis input_hash 必须变化。
3. content_analysis 输出变化后，ai_voiceover_candidate_materialize input_hash 必须变化。
4. ai_voiceover_candidate_materialize 输出变化后，short_video_edit_plan input_hash 必须变化。
5. short_video_edit_plan 输出变化后，voiceover_script input_hash 必须变化。
```

建议 hash 组成：

```text
asr_micro_segment input_hash = source_raw_asr_index_hash + prompt_version + asr_micro_segment config
content_analysis input_hash = asr_micro_segment hash + content_analysis prompt_version + content_analysis config
candidate_materialize input_hash = content_analysis hash + asr_micro_segment hash + ai_voiceover_candidate config
short_video_edit_plan input_hash = ai_voiceover_candidate_materialize hash + prompt_version + edit_plan config
voiceover_script input_hash = short_video_edit_plan hash + prompt_version + voiceover config
```

否则会出现：

```text
raw_asr_index 已更新，但 asr_micro_segment / content_analysis 复用旧缓存，最后看起来跑通，实际仍然用旧事实和旧时间。
```
...
2. AGENT_INFO["content_analysis"] 是否已经绑定 CONTENT_ANALYSIS_MICRO_SEGMENT_PROMPT。
3. pipeline.py 中是否已经实现 step_ai_voiceover_candidate_materialize handler。
4. handler 是否真正 fail-fast，而不是空结果 success。
5. asr_micro_segment / content_analysis / short_video_edit_plan / voiceover_script 是否仍存在旧 fallback。
6. content_analysis 输入 prompt 是否仍要求“只输出 segment_id”或给出 time 时间码。
7. candidate_materialize 是否仍会截断 overlong，或为了 min_clip_seconds 扩展事实边界。
8. short_video_edit_plan 是否只读取 ai_voiceover_candidate_materialize 的 candidate_clips。
9. semantic retry 是否保留首次 raw_response，并且 retry 失败后不会降级成功。
10. debug / diagnostics / output_files 是否完整写入 manifest 和失败导出包。
```
...
## 12.14 `content_analysis` prompt 输入协议作为硬验收

当前分支即使已经把 `AGENT_INFO["content_analysis"]` 绑定到 micro_segment prompt，也还要检查实际构造给模型的文本。

必须满足：

```text
1. 输入展示标识为 micro_segment_id。
2. 输入不再写“只输出 segment_id”。
3. 输入不再给 time=source_start-source_end。
4. 输入不出现 chunk_id。
5. 输出格式只允许 selected_segments[].micro_segment_id。
```

允许保留：

```text
1. source_id：只用于区分多源素材，不允许模型输出。
2. duration：只用于判断片段长短。
3. 声：ASR 文本。
4. 画：视觉上下文。
```

如果 normalize 继续兼容 `segment_id`，必须写 diagnostics：

```text
legacy_segment_id_used > 0 时不一定失败，但必须暴露。
如果 production_mode=ai_voiceover 且旧字段使用比例过高，建议 fail-fast，避免旧 prompt 污染新版链路。
```
...
## 13.6 child ASR 改变后的缓存失效测试

构造方式：

```text
修改某个 child asr_segments.json 的文本或时间。
```
...
```text
source_aggregate 必须重新生成 source_raw_asr_index.json。
asr_micro_segment input_hash 必须变化。
content_analysis input_hash 必须变化。
不得复用旧 micro_segments / selected_segments / candidate_clips。
```

## 13.7 legacy CLI 单源误入测试

构造方式：

```text
直接 run_pipeline.py --input xxx.mp4，不提供 source_manifest。
```

预期：

```text
如果进入 ("ai_voiceover", false)，必须明确提示 legacy 单源不属于本轮验收。
不能自动 fallback 到 timeline_digest.speech。
不能静默拼 raw_asr_index。
不能误伤 source_analysis 创建的 common_only 子任务。
```
...
```text
让 AGENT_INFO["content_analysis"] 仍绑定旧 CONTENT_ANALYSIS_PROMPT。
```

预期：

```text
失败在 content_analysis 启动前。
错误码：content_analysis_prompt_binding_invalid。
不能让旧 prompt 输出 candidate_clips 后继续进入新版链路。
```

## 13.9 `candidate_materialize` 短片段不自动扩展测试

构造方式：

```text
让某个 selected micro_segment 加 padding 后 duration < min_clip_seconds。
```

预期：

```text
candidate_materialize 不自动 padded_end += extra。
不向后吞下一句，不向前吞上一句。
materialize_debug.json 记录 short_clips。
fail_on_short=false 时可以继续，但必须保持原事实边界。
fail_on_short=true 时必须 fail-fast。
```

## 13.10 raw ASR 与 chunk 无 overlap 诊断测试

构造方式：

```text
构造一个 ASR segment 时间落在 chunk 范围外，或人为修改 chunk 时间让 overlap=0。
```

预期：

```text
source_raw_asr_index.json 中该 ASR 的 primary_chunk_id 为空。
不伪造 chunk_id。
diagnostics 记录 asr_chunk_no_overlap、source_id、asr_start_seconds、asr_end_seconds、nearest_chunk_id、nearest_distance_seconds。
asr_micro_segment 可以继续使用 ASR 时间生成 micro_segment，但 visual_context 可为空。
```
...
开发完成后按下面红线验收。只要任意一条不满足，就说明新版 micro_segment 主链路仍然不可靠。

```text
1. asr_micro_segment 不允许从 timeline_digest.speech 构造时间。
2. asr_micro_segment 模型 groups 为空或 valid_groups 为空时必须 fail-fast，不允许默认 fallback 生成全部 sentence。
3. content_analysis 不允许输出 candidate_clips 给下游使用。
4. content_analysis 输入/输出主协议必须是 micro_segment_id，不允许 prompt 继续写“只输出 segment_id”或给 time 时间码。
5. ai_voiceover_candidate_materialize 是 AI 配音 candidate_clips 唯一来源。
6. candidate_materialize 不允许为了 max_clip_seconds 截断事实边界。
7. candidate_materialize 不允许为了 min_clip_seconds 扩展事实边界。
8. short_video_edit_plan 不允许从 content_analysis 读取候选。
9. short_video_edit_plan / voiceover_script 合法 JSON 但业务为空时，必须 semantic retry 或 fail-fast。
10. tts outputs 为空时，require_tts=true 必须失败。
11. cut_plan clips 为空必须失败。
12. highlight_reassembly workflow 不得出现 asr_micro_segment / ai_voiceover_candidate_materialize。
13. 所有调试 fallback 默认 false，开启时必须写 diagnostics。
14. 任何步骤失败后，下游不得读取旧 success 产物继续跑。
15. source_raw_asr_index 变化后，下游缓存必须逐级失效。
16. child manifest 缺 asr_output_hash 时，必须读取 child ASR 文件内容计算 hash。
17. 失败导出包必须能看出 resolved_workflow_key、resolved_workflow_steps 和 effective_runtime_flags。
```
...
3. 修 candidate_materialize 不为了 min_clip_seconds 扩展事实边界，只记录 short_clips。
4. 统一 content_analysis micro_segment_id 文案，删除 segment_id/time 输出暗示。
5. 加 short_video_edit_plan 语义 retry，仍空则 fail-fast。
6. 确认 voiceover_script / tts / cut_plan 空结果不会 success。
