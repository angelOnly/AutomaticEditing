...
2. content_analysis 不允许输出 candidate_clips 给下游使用。
3. ai_voiceover_candidate_materialize 是 AI 配音 candidate_clips 唯一来源。
4. short_video_edit_plan 不允许从 content_analysis 读取候选。
5. short_video_edit_plan / voiceover_script 合法 JSON 但业务为空时，必须 semantic retry 或 fail-fast。
6. tts outputs 为空时，require_tts=true 必须失败。
7. cut_plan clips 为空必须失败。
8. highlight_reassembly workflow 不得出现 asr_micro_segment / ai_voiceover_candidate_materialize。
9. 所有调试 fallback 默认 false，开启时必须写 diagnostics。
10. 任何步骤失败后，下游不得读取旧 success 产物继续跑。
11. source_raw_asr_index 变化后，下游缓存必须逐级失效。
12. 失败导出包必须能看出 resolved_workflow_key、resolved_workflow_steps 和 effective_runtime_flags。
```
