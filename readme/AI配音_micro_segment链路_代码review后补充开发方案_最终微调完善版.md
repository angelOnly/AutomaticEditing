...
10. config.toml 当前可能包含真实 API key，提交前必须清理并改为环境变量或本地私有配置。
```
...
2. ("ai_voiceover", True) workflow 已加入 ai_voiceover_candidate_materialize。
3. step_ai_voiceover_candidate_materialize() 已存在，并会写出 ai_voiceover_candidates/v*/candidate_clips.json 与 materialize_debug.json。
...
7. config.toml 中如存在真实 API key，提交前必须移出仓库并轮换；如果 key 已经推送到公开仓库，必须立即轮换，不能只靠删除文件解决。
```
...
原因：本次输入协议已经从 timeline_digest / aggregate 内嵌 raw_asr_index 收敛为独立 `source_raw_asr_index.json`，属于缓存语义变化。即使 prompt 文案没有大改，prompt_version 也不应继续停留在 `asr_micro_segment_v1`。
...
# P0-4. 提交前清理 config.toml 中的真实 API key
